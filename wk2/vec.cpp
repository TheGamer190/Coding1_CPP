...
int main()
{
    ...
    std::vector<std::string> favoriteGames = {"Persona", "Minecraft", "Genshin"};
    ...
    do{
        ...
        {
            std::cout << "Removing and element from a global vector\n";

            sort(favoriteGames.begin(), favoriteGames.end());
            for(int i = 0; i < favoriteGames.size(); i++)
            {
                std::cout << favoriteGames[i] << std::endl;
            }
            std::cout << "What name should we remove from faves?\n";
            std::string favInput;
            std::cin >> favInput;

            auto iter = find(favoriteGames.begin(), favoriteGames.end(), favInput);
            if(find(favoriteGames.begin(), favoriteGames.end(), favInput) != favoriteGames.end())
            {
                std::cout << "Found it\n";
